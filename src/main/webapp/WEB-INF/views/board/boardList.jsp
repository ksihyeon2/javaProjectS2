...
<head>
  ...
  <script>
  	...
  	function partCheck(){
  		let part = $("#part").val();
  		location.href="boardList?part="+part;
  	}
  	...
  	function searchCheck(){
  		...
  		location.href="boardList?search="+search+"&searchString="+searchString;
  	}
  </script>
</head>
<body>
...
<div class="container">
	...
	<form name="searchForm">
		<div class="row mb-5">
	        ...
	        <option ${part == "공지" ? "selected" : ""}>공지</option>
  ...
  <table class="table table-hover text-center">
  	<tr style="background-color:rgb(213,213,213);height:40px;">
		...
		<c:forEach var="noticeVO" items="${noticeVOS}">
	  	<tr class="notice">
	  		<td style="color:red"><i class='fas fa-bullhorn' style='font-size:20px'></i></td>
  			<td>${noticeVO.nickName}</td>
  			<td><a href="boardContent?idx=${noticeVO.idx}">${noticeVO.title}</a></td>
  			<td>${noticeVO.part}</td>
  			<td>
  				<c:if test="${noticeVO.hour_diff > 24}">${fn:substring(noticeVO.WDate,0,10)}</c:if>
  				<c:if test="${noticeVO.hour_diff <= 24}">${noticeVO.date_diff == 0 ? fn:substring(noticeVO.WDate,11,16) : fn:substring(noticeVO.WDate,0,16)}
  				<span class="badge badge-danger" style="font-size:0.5em">N</span>
					</c:if>
  			</td>
  			<td>${noticeVO.readNum}</td>
  		</tr>
		</c:forEach>
  		...
  		<td>
  			<table class="table table-hover text-center">
  				<tr style="background-color:rgb(213,213,213);height:40px;">
  					...
  					<c:forEach var="vo" items="${noticeVOS}" varStatus="st">
	  					<tr>
  							<td>${st.count}</td>
